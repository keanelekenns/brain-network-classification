...
def localSearchOQC(G, alpha, t_max=1000, seed=None):
    ...
    while b1 and t <= t_max:
        ...
        while b2:
            ...
            for node in G:
                if node not in S:
                    new_weight = weight + sum(1 for neighbor in G.neighbors(node) if neighbor in S)
...
def greedyOQC(G, alpha):
    ...
    for node in G:
        ...
        neighbors[node] = {neighbor: 1 for neighbor in G.neighbors(node)}
...
def find_seed_negative(G):
    ...
    best_density = sum(data['weight'] for u, v, data in best_ego.edges(data=True)) / float(len(best_ego))
    for node in G:
        ego = nx.ego_graph(G, node)
        density = sum(data['weight'] for u, v, data in ego.edges(data=True)) / float(len(ego))
...
def localSearchNegativeOQC(G, alpha, t_max=1000, seed=None):
    ...
    if seed is None:
        ...
        S = seed
    weight = sum(data['weight'] for u, v, data in S.edges(data=True))
    N = float(len(S))
    oqc_score = weight - alpha * (N * (N - 1) / 2)
    b1 = True
    t = 1
    while b1 and t <= t_max:
            ...
            for node in G:
                if node not in S:
                    new_weight = weight + sum(G[node][neighbor]['weight'] 
                                              for neighbor in G.neighbors(node) if neighbor in S)
                    new_oqc_score = new_weight - alpha * (N * (N + 1) / 2)
                    if new_oqc_score > oqc_score:
                        weight = new_weight
                        oqc_score = new_oqc_score
        ...
        if len(S) == 1:
            break
        for node in S.nodes():
            new_weight = weight - sum(S[node][neighbor]['weight'] for neighbor in S.neighbors(node))
            new_oqc_score = new_weight - alpha * ((N - 1) * (N - 2) / 2)
            if new_oqc_score > oqc_score:
                weight = new_weight
                oqc_score = new_oqc_score
        ...
        t += 1
    S_bar = G.subgraph([node for node in G if node not in S])
    oqc_score_bar = (sum(data['weight'] for u, v, data in S_bar.edges(data=True)) - 
